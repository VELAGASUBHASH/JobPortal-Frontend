...
const fetchJob = async ({ queryKey }) => {
  ...
  const { data } = await axios.get(`https://jobportal-backend-g2k3.onrender.com/api/jobs/${id}`);
  return data;
};
...
const JobDetail = () => {
  ...
  const handleResumeUpload = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`https://jobportal-backend-g2k3.onrender.com/api/ai/parse-resume`, formData, {
  ...
  const handleApply = async (e) => {
    ...
    try {
      await axios.post(`https://jobportal-backend-g2k3.onrender.com/api/jobs/${id}/apply`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
      setApplied(true);
